...
The band-selection scene SHALL present exactly five selectable band characters: blink-182, Green Day, Linkin Park, Sum 41, and Avril Lavigne.

#### Scenario: Roster is displayed
- **WHEN** the band-selection scene is shown
- **THEN** all five band characters are displayed as selectable options with a name/label and a pixel-art character representation

### Requirement: Selecting a band
The player SHALL be able to select one band character, and that selection SHALL lock in the band theme (character, logo, and song) for the session.
...
- **THEN** the currently focused option is visually highlighted before confirmation
